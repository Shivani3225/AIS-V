...
const BlogPost = () => {
  ...
  // Upload blog image - UPDATED to match web version
  const uploadBlogImage = async (file) => {
    const formData = new FormData();
    formData.append("file", {
      uri: file.uri,
      type: file.type || "image/jpeg",
      name: file.fileName || "blog-cover.jpg",
    });
    formData.append("folder", "blogs"); // Add folder parameter

    try {
      const response = await apiRequest("post", "/vendor-profile/upload-image", formData, true);
  ...
  const handleSave = async (statusToSave) => {
    ...
    try {
      ...
      Alert.alert("Error", error.response?.data?.message || error.message || "Failed to save blog");
    } finally {
      setSaving(false);
    }
  };
