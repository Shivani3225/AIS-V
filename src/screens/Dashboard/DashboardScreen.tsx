...
// });
import React, { useState, useEffect } from "react";
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, ActivityIndicator, Alert, RefreshControl, Modal } from "react-native";
import LinearGradient from "react-native-linear-gradient";
import Icon from "react-native-vector-icons/Feather";
import CustomHeader from '../../components/header/CustomHeader';
import AsyncStorage from '@react-native-async-storage/async-storage';
...
const DashboardScreen = () => {
  ...
  const fetchVendorSubscription = async (vendorId) => {
    ...
    try {
      ...
      const endpoints = [
        ...
        `${API_BASE_URL}/vendor-profile/profile/me`,
        `${API_BASE_URL}/vendors/${vendorId}/subscription`
      ];
  ...
  const handleLogout = async () => {
    try {
      ...
      await AsyncStorage.removeItem("vendorBusinessName");
      await AsyncStorage.removeItem("vendorEmail");
      ...
      try {
        ...
        console.log("Google sign out error (may not be logged in with Google):", googleError);
      }
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <View style={styles.modalIconContainer}>
              <Icon name="log-out" size={50} color="#EF4444" />
            </View>
